...
const SocialProof: React.FC = () => {
  ...
  return (
    <section ref={sectionRef} className="pt-12 sm:pt-16 md:pt-20">
      <div className="text-center mb-8 sm:mb-12 relative">
        <h2 className={`reveal-fade-up reveal-stagger-1 ${isVisible ? 'visible' : ''} text-2xl sm:text-3xl font-black italic tracking-tighter uppercase mb-2 relative`}>
          {t('social.realPlayers')}
        </h2>
        <p className={`reveal-fade-up reveal-stagger-2 ${isVisible ? 'visible' : ''} text-gray-400 text-xs sm:text-sm font-medium`}>
          ...
          {testimonials.map((testimonial, idx) => (
            <div key={idx} className="bg-gradient-to-br from-gray-900 to-gray-950 border border-pickwin-green/20 p-6 rounded-3xl flex flex-col justify-between hover:bg-gray-800 transition-all shadow-xl flex-shrink-0 relative overflow-hidden group" style={{ width: 'calc(100vw - 3rem)' }}>
              ...
              <div>
                ...
                <p className="text-white font-medium mb-6 leading-relaxed italic">"{testimonial.text}"</p>
              </div>
              <div>
                <p className="text-white font-black italic text-lg">{testimonial.name}</p>
              </div>
            </div>
          ))}
        </div>
      ...
      <div className="hidden md:grid md:grid-cols-3 gap-4 max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
        {testimonials.map((testimonial, i) => (
          <div key={i} className={`bg-gradient-to-br from-gray-900 to-gray-950 border border-pickwin-green/20 p-6 rounded-3xl flex flex-col justify-between hover:bg-gray-800 transition-all shadow-xl relative overflow-hidden group reveal-fade-up ${i < 3 ? `reveal-stagger-${i + 1}` : `reveal-stagger-${i - 2}`} ${isVisible ? 'visible' : ''}`}>
            <div className="absolute inset-0 bg-gradient-to-br from-pickwin-green/5 via-transparent to-pickwin-green/5 opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none"></div>
            <div className="absolute top-2 right-2 w-8 h-8 opacity-15 pointer-events-none bandana-pattern border border-white/20"></div>
            <div>
              <div className="flex gap-1 mb-4 relative z-10">
